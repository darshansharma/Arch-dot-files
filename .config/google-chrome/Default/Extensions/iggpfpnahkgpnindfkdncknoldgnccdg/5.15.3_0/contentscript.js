...
if (typeof(chrome.runtime) == 'object') {
  ...
  if (localStorage['yt-loop-show-changelog'] == "true" && localStorage['yt-loop-version'] && localStorage['yt-loop-version'] != thisVersion.toString()) {
    ...
    var changelog_url = "http://www.alvinhkh.com/looperforyoutube/changelog/updated";
...
updateButton: function() {
  ...
  button.setAttribute('class', 'style-scope ytd-toggle-button-renderer');
  if (ytl.getOption('buttonIcon')) {
    var iconContainer = document.createElement('button');
    iconContainer.setAttribute('id', 'button');
    iconContainer.setAttribute('is', 'paper-icon-button-light');
    iconContainer.setAttribute('class', 'icon-container style-scope ytd-button-renderer style-default');
  ...
  if (ytl.getOption('buttonText')) {
    ...
    text.setAttribute('class', 'style-scope ytd-button-renderer style-default');
...
setButton: function() {
  ...
  if (ytl.layout == '2017' || ytl.layout == '2016') {
    ...
    renderer.setAttribute('class', 'loop-button style-scope ytd-menu-renderer style-default-active force-icon-button');
...
keydownAction: function (e) {
  ...
  if (!ytl.getOption('shortcut-pause') && e.keyCode == 32) return; 
  switch(e.keyCode) {
    case 80: // p
    case 82: // r
...
ytl.titleObserver = new MutationObserver(function (mutations) {
  mutations.forEach(function (mutation) {
      ...
      var isYouTube_host = (host.substr(host.length - 11) == 'youtube.com' && host != 'm.youtube.com');
      var isYouTube_target = ((mutation.target.baseURI).match("youtube.com") != null);
      if (document.querySelector('ytd-watch') || document.querySelector('ytg-app')) {
        if (mutation && mutation.target && isYouTube_host && isYouTube_target) {
          if ((mutation.target.baseURI).match("watch\\?") != null) {
            if (document.querySelector('ytd-app')) {
              var videoId = document.querySelector('ytd-watch').getAttribute('video-id');
